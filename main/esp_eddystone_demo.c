...
#define OPEN_DOORS 35
#define CLOSE_DOORS 36
...
static void open_door(void)
{
    //ESP_LOGE("CENTRAL","PORTAS ABERTAS");
    gpio_set_level(CLOSE_DOORS,0);
    gpio_set_level(OPEN_DOORS,1);

}
static void close_door(void)
{
    //ESP_LOGE("CENTRAL","PORTAS FECHADAS");
    gpio_set_level(CLOSE_DOORS,1);
    gpio_set_level(OPEN_DOORS,0);
}
static void esp_eddystone_show_inform(const esp_eddystone_result_t* res, int rssi)
{
    switch(res->common.frame_type)
    {
        case EDDYSTONE_FRAME_TYPE_UID: {
            ...
            {                
                ...
                //printf("%02X ",res->inform.uid.namespace_id[i]);
                i++;
            }
            if(tr == i && rssi > -65)
            {
...
void config_pins()
{
    gpio_pad_select_gpio(OPEN_DOORS);
    gpio_pad_select_gpio(CLOSE_DOORS);
    gpio_set_direction(OPEN_DOORS,GPIO_MODE_OUTPUT);
    gpio_set_direction(CLOSE_DOORS,GPIO_MODE_OUTPUT);
    gpio_pulldown_en(OPEN_DOORS);
    gpio_pulldown_en(CLOSE_DOORS);
}
void vDoorTask(void *pvParameter)
{
    for(;;){
        gettimeofday(&ctm,NULL);
        f = ctm.tv_sec;
...
void app_main(void)
{
    ...
    xTaskCreate(&vDoorTask,"DOOR TASK",2048,NULL,1,NULL);
